...
def update_course(student_id:int, sifra_predmeta:str, datum :datetime, exam : schemas.ExamBase, exam_service: ExamServices = Depends(get_exam_service), token : str = Depends(oauth2_scheme)):
    user_data = verify_access_token(token)
...
def delete_course(student_id:int , sifra_predmeta:str , datum :datetime, exam_service: ExamServices = Depends(get_exam_service),token : str = Depends(oauth2_scheme)):
    user_data = verify_access_token(token)
